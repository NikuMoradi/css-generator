        ...
        <div style="padding: 0 1rem">
          ...
          <div v-if="bg_selection === 'Single Color / Image'">
            <q-input
              label="Color"
              outlined
              v-model="bg_backgroundColor"
              class="mb-10"
            >
              <template v-slot:append>
                <div
                  class="show-color"
                  :style="{ backgroundColor: bg_backgroundColor }"
                ></div>
                <q-popup-proxy
                  cover
            ...
            <div class="col-xs-6 col-sm-3 col-md-3">
              <q-input label="Color" outlined v-model="bg_gradient_color_1">
                <template v-slot:append>
                  <div
                    class="show-color"
                    :style="{ backgroundColor: bg_gradient_color_1 }"
                  ></div>
                  <q-popup-proxy
                    cover
                    transition-show="scale"
            ...
            <div class="col-xs-6 col-sm-3 col-md-3">
              <q-input label="Color" outlined v-model="bg_gradient_color_2">
                <template v-slot:append>
                  <div
                    class="show-color"
                    :style="{ backgroundColor: bg_gradient_color_2 }"
                  ></div>
                  <q-popup-proxy
                    cover
                    transition-show="scale"
          ...
          >
            ...
            <div class="col-6">
              <q-select
                outlined
                v-model="bg_image_size"
                :options="bg_image_sizes"
                label="Size"
              />
            </div>
            <div class="col-6">
            ...
            <div class="col-6">
              <q-select
                outlined
                v-model="bg_image_clip"
                :options="bg_image_clips"
                label="Clip"
              />
            </div>
          </div>
...
export default {
  ...
  setup(props, ctx) {
    ...
    const bg_gradient_options = ref([
      ...
      "linear-gradient(120deg, #fbc2eb 0%, #a6c0fe 100%)", // Pastel Purple
      "linear-gradient(120deg, #f6d365 0%, #fda085 100%)", // Warm Gradient
      "linear-gradient(90deg, #fbc2eb 0%, #a6c0fe 100%)", // Soft Purple
      ...
      "linear-gradient(120deg, #fbc2eb 0%, #a6c0fe 100%)", // Pink to Blue
      "linear-gradient(180deg, #fdfbfb 0%, #ebedee 100%)", // Light Gray
      "linear-gradient(45deg, #ff9a9e 0%, #fad0c4 100%)", // Warm Pink
      "linear-gradient(180deg, #f6d365 0%, #fda085 100%)", // Sunset Gradient
      "linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%)", // Blue to White
      "linear-gradient(60deg, #abecd6 0%, #fbed96 100%)", // Mint to Yellow
      "linear-gradient(120deg, #fbc2eb 0%, #a6c0fe 100%)", // Purple to Pink
      "linear-gradient(180deg, #ffecd2 0%, #fcb69f 100%)", // Cream to Pink
      "linear-gradient(90deg, #f093fb 0%, #f5576c 100%)", // Purple to Pink
      "linear-gradient(180deg, #ff5f6d 0%, #ffc371 100%)", // Coral to Yellow
      "linear-gradient(90deg, #f3ec78 0%, #af4261 100%)", // Yellow to Green
      "linear-gradient(120deg, #ff9a9e 0%, #fad0c4 100%)", // Peach to Pink
      "linear-gradient(60deg, #ff6b6b 0%, #f06595 100%)", // Pink to Purple
      "linear-gradient(135deg, #a1c4fd 0%, #c2e9fb 100%)", // Blue to White
      "linear-gradient(120deg, #a18cd1 0%, #fbc2eb 100%)", // Purple to Pink
      "linear-gradient(180deg, #ffecd2 0%, #fcb69f 100%)", // Cream to Peach
      "linear-gradient(180deg, #ff5f6d 0%, #ffc371 100%)", // Coral to Yellow
      "linear-gradient(60deg, #abecd6 0%, #fbed96 100%)", // Mint to Yellow
      "linear-gradient(120deg, #fbc2eb 0%, #a6c0fe 100%)", // Pink to Blue
      "linear-gradient(120deg, #f6d365 0%, #fda085 100%)", // Warm Gradient
    ...
    watch(bg_gradient_option, (newVal) => {
      if (newVal) {
        ...
        const gradientDetails = newVal.match(
        ...
        if (gradientDetails) {
          ...
          if (type === "linear-gradient") {
            ...
            if (colorStops.length > 0) {
              ...
              bg_gradient_position_1.value = position1
                ? position1.replace("%", "")
                : "0";
            }
            if (colorStops.length > 1) {
              ...
              bg_gradient_position_2.value = position2
                ? position2.replace("%", "")
                : "100%";
            }
    ...
    const backgroundCode = computed(() => {
      ...
      if (bg_selection.value === "Single Color / Image") {
        ...
        if (bg_gradient_type.value === "linear-gradient" && isLinear.value) {
          ...
          bg_gradient_type.value === "radial-gradient" &&
          isRadial.value
        ) {
    ...
    const parseGradientString = (gradientString) => {
      const gradientParts = gradientString.match(
        /(linear-gradient|radial-gradient)\(([^)]+)\)/
      );
      if (gradientParts) {
