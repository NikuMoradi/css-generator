    ...
    <div class="col-xs-12 col-sm-12 col-md-6">
      <div class="no-scroll-area">
        <q-input
          label="Color"
          outlined
          v-model="filter_imageShadow_color"
          class="mb-10"
        >
        ...
        <div class="outlined-slider mb-10">
          <div class="slider-label">
            ...
            ><span>{{ filter_imageShadow_vertical_offset }} px </span>
          </div>
          <q-slider
...
export default {
  ...
  setup(props, ctx) {
    ...
    const parseDropShadow = (filter) => {
      const regex =
        /drop-shadow\(\s*(-?\d+)px\s+(-?\d+)px\s+(\d+)px\s*(#[0-9a-fA-F]{6})?\s*\)/;
...
<style scoped>
.drop-shadow-preview {
  background: #ffff url("/src/assets/components/style/sample2.jpg") center
