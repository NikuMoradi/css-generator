      ...
      <div class="preview-box">
        <div class="prespective-preview" :style="perspectivePreview_parent">
          <div :style="perspectivePreview_child">
            ...
            dolore quae saepe ipsum iste necessitatibus sunt quisquam rerum,
            unde vel eveniet impedit ratione soluta recusandae? Voluptas dolor
            ipsam aspernatur excepturi porro!
          </div>
        </div>
      </div>
      <div class="code-box">
  ...
  setup(props, ctx) {
    ...
    const perspectiveCode_parent = computed(() => {
      if (
        ...
        perspective_length.value !== undefined
      ) {
    ...
    Object.entries(props.perspectiveProp).forEach(([key, value]) => {
      switch (key) {
        case "perspective":
          ...
          break;
        case "transform":
          const transformMatch = value.match(
            /rotateX\((\d+)deg\)?\s*rotateY\((\d+)deg\)?/
          );
